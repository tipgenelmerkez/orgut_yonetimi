...
#İlçeler
class OrgutCity(models.Model):
    _name = 'orgut.city'
    _description = 'İlçe Örgütü'
    _inherit = ['mail.thread']
    _sql_constraints = [
        ('unique_city_state', 'unique(name, state)', 'Bu ilçe Örgütü zaten mevcut')
    ]

    name = fields.Many2one('res.city', 'City', required=True)
    state = fields.Many2one('orgut.state', 'State', required=True)
    state_id = fields.Many2one(related='state.name')
    region = fields.Many2one('orgut.region', 'Region', required=True)
    country = fields.Many2one('res.country', 'Country', default=lambda self: self.env['res.country'].browse(224), required=True)
    ...
    def create(self, vals):
        city = super(OrgutCity, self).create(vals)
        city._assign_internal_user_access()
        utils.update_followers(city)
        return city

    def write(self, vals):
        old_members = set(self.member_line_ids.ids)
        result = super(OrgutCity, self).write(vals)
    ...
    def _check_unique_city_state(self):
        for record in self:
            existing_city = self.search(
                [('name', '=', record.name.id), ('state', '=', record.state.id), ('id', '!=', record.id)],
                limit=1)
            if existing_city:
                raise exceptions.ValidationError('Bu İlçe Örgütü Zaten Mevcut.')
